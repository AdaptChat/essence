...
pub trait GuildDbExt<'t>: DbExt<'t> {
    ...
    async fn build_guild_cache(&self) -> crate::Result<()> {
        ...
        cache::write().await.existing_guild_ids = Some(guild_ids);
        Ok(())
    }
    ...
    async fn fetch_member_permissions(
        ...
        if let Some(permissions) = cached_permissions {
            ...
            let guild = cache
                ...
                .entry(guild_id)
                .or_insert_with(Default::default);
            let member_perms = guild
                .member_permissions
                .entry(user_id)
                .or_insert_with(Default::default);
            member_perms.insert(channel_id, perms);
    ...
    async fn fetch_observable_user_ids_for_user(&self, user_id: u64) -> crate::Result<Vec<u64>> {
        let user_ids = sqlx::query!(
            ...
            FROM
                members
            WHERE guild_id IN (SELECT guild_id FROM members WHERE id = $1);
            "#,
            user_id as i64,
        )
        .fetch_all(self.executor())
